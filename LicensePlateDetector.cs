...
namespace LicensePlateRecognition {
    public class LicensePlateDetector : DisposableObject {
        ...
        public LicensePlateDetector(string dataPath) {
            // Kreiraj OCR engine koristeći TesseractEngine
    ...
    List<String> licenses) {
            for (; idx >= 0; idx = hierachy[idx, 0]) {
                ...
                using (VectorOfPoint contour = contours[idx]) {
                    if (CvInvoke.ContourArea(contour) > 400) {
                        ...
                        using (UMat plate = GetLicensePlateRegion(gray, box))
                        using (UMat filteredPlate = FilterPlate(plate)) {
                            string recognizedText = RecognizeText(filteredPlate);
                            if (!string.IsNullOrWhiteSpace(recognizedText)) {
                                licenses.Add(recognizedText);
        ...
        private string RecognizeText(UMat filteredPlate) {
            string recognizedText = string.Empty;
            using (var pix = PixConverter.ToPix(filteredPlate.ToBitmap())) {
        ...
        private void AdjustBoxOrientation(ref RotatedRect box) {
            if (box.Angle < -45.0) {
        ...
        private UMat GetLicensePlateRegion(IInputArray gray, RotatedRect box) {
            UMat plate = new UMat();
        ...
        new PointF(box.Size.Width - 1, box.Size.Height - 1)
            ...
            using (Mat rot = CvInvoke.GetAffineTransform(srcCorners, destCorners)) {
                CvInvoke.WarpAffine(gray, plate, rot, Size.Round(box.Size));
            }
